...
module.exports = class RTCSession extends EventEmitter
{
      ...
      if (! this._late_sdp)
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          .then(() => new Promise((resolve, reject) =>
            this._connection.setRemoteDescription(offer, resolve, reject)))
      ...
      switch (request.method)
      {
        case JsSIP_C.ACK:
          ...
          if (this._late_sdp)
          {
            ...
            this._connectionPromiseQueue = this._connectionPromiseQueue
              .then(() => new Promise((resolve, reject) =>
                this._connection.setRemoteDescription(answer, resolve, reject)))
              .then(() =>
              {
  ...
  _createLocalDescription(type, onSuccess, onFailure, constraints)
  {
    ...
    if (type === 'offer')
    {
      this._connectionPromiseQueue = this._connectionPromiseQueue
        .then(() => new Promise((resolve, reject) =>
          connection.createOffer(resolve, reject, constraints)))
    ...
    else if (type === 'answer')
    {
      this._connectionPromiseQueue = this._connectionPromiseQueue
        .then(() => new Promise((resolve, reject) =>
          connection.createAnswer(resolve, reject, constraints)))
        .then(createSucceeded.bind(this))
        .catch((error) =>
        {
          this._rtcReady = true;
  ...
  _receiveReinvite(request)
  {
    ...
    if (request.body)
    {
      ...
      const setSdp = () =>
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          .then(() => new Promise((resolve_, reject_) =>
            this._connection.setRemoteDescription(offer, resolve_, reject_)))
      ...
      case /^2[0-9]{2}$/.test(response.status_code):
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          .then(() =>
          {
            ...
            {
              return new Promise(
                (resolve, reject) => this._connection.createOffer(resolve, reject)
              )
                .then((offer) => new Promise((resolve, reject) =>
                  this._connection.setLocalDescription(offer, resolve, reject))
                )
                .catch((error) =>
                {
